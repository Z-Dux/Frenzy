import { tradeConfig } from "@config/app";
import { binance, type Binance } from "@exchange/binance";
import { orm } from "@db/index";
import { closeTrade, openTrade } from "@discord/bot/handlers";
import { PortfolioSchema, TradeSchema } from "@db/schema";
...
export class Trader {
  ...
  async addTrade(trade: Trade, create = false) {
    if (trade.status !== "closed") {
      this.binance.subscribe(trade.asset).catch(console.error);
    }
    ...
    if (create) {
      try {
        ...
        console.error("Failed to create trade in database:", err);
      }
    }
  ...
  async removeTrade(trade: Trade) {
    try {
      ...
      const assetExists = this.portfolio.trades.filter(
        (t) => t.asset === trade.asset && t.status !== "closed",
      );

      if (assetExists.length === 0) {
        this.binance.unsubscribe(trade.asset);
      }
    } catch (err) {
      console.error(`Failed to remove trade for ${trade.asset}:`, err);
    }
  }
  watchTrades() {
      ...
      try {
        ...
        for (const trade of trades) {
          ...
          if (
            ...
            if (priceDiff === 0) {
              console.error("Invalid trade: entry == stop_loss");
              continue;
            }
          ...
          if (updated) {
            try {
              ...
              console.error(
                `Failed to update trade in database for ${trade.messageUrl}:`,
                err,
              );
            }
          }
        ...
        console.error("Error in watchTrades:", err);
      }
    });
  }
}
